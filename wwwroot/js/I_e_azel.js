        ...
        success: function (response) {
          ...
          function generarContenidoPopup(coordenada) {
            ...
            if (camposVisiblesGlobal.includes("RazonSocial")) {
              ...
              contenido +=
                "<h2 class='razon-social-popup'><img src='" +
                imgSrc +
          ...
          // Configuración base para los iconos
          var iconoBase = L.Icon.extend({
            options: {
              iconSize: [36, 36],
              iconAnchor: [12, 18],
          ...
          for (var j = 0; j < response.length; j++) {
            ...
            switch (coordenada.clasificacion) {
              ...
              case "Eólica":
                iconoActual = iconos.eolica; break;
              case "Turbo Gas y Vapor":
                iconoActual = iconos.vapor; break;
              case "Combustión Interna":
                iconoActual = iconos.combustion; break;
              case "Cogeneración":
                iconoActual = iconos.cogeneracion; break;
              case "Cogeneración Eficiente":
                iconoActual = iconos.cogeneracionEficiente; break;
              case "Ciclo combinado":
                iconoActual = iconos.cicloCombinado; break;
              case "Carboeléctrica":
                iconoActual = iconos.carboelectrica; break;
              case "Solar Fotovoltaica":
                iconoActual = iconos.solar; break;
              case "Energía Cinética":
                iconoActual = iconos.cinetica; break;
              case "Nucleoeléctrica":
                iconoActual = iconos.nuclear; break;
              case "Lecho Fluidizado":
                iconoActual = iconos.lecho; break;
              case "Importación":
                iconoActual = iconos.importacion; break;
              case "Geotérmica":
                iconoActual = iconos.geotermica; break;
              default:
                iconoActual = iconos.convencional;
            }
...
///Carga Shapes
// Declarar las capas globalmente para poder acceder y eliminarlas según la selección
var regionesLayer = null;
var estadosLayer = null;
var municipiosLayer = null;
var capaRasterViento = null; // Añadimos el raster como capa global

function limpiarTodo() {
  // Limpiar marcadores, clústeres y círculos
  map.eachLayer(function (layer) {
    if (
      // layer instanceof L.Marker ||
      // layer instanceof L.MarkerClusterGroup ||
  ...
  // Reiniciar las referencias de las capas a null
  estadosLayer = null;
  municipiosLayer = null;
  regionesLayer = null;
  capaRasterViento = null;
}

function cargarTerminosBusqueda() {
  availableTerms = [];  // Reseteamos los términos

  if (capaActiva === 'estados' && estadosLayer) {
    estadosLayer.eachLayer(function (layer) {
  ...
  if (capaActiva === 'municipios' && municipios_mapa) {
    for (var i = 0; i < municipios_mapa.features.length; i++) {
      var municipio = municipios_mapa.features[i].properties.NOM_MUN;
      var estado = municipios_mapa.features[i].properties.NOMGEO;

      if (municipio && estado) {
        availableTerms.push(municipio + ", " + estado);
      }
    }
  }

  // Actualiza el autocompletado si hay términos disponibles
  if (availableTerms.length > 0) {
    autocomplete(document.getElementById("busquedaGeneralInput"), availableTerms);
  } else {
    // Si no hay términos, limpia el autocompletado
...
function loadShape(shapeType) {
  limpiarTodo();  // Aquí estaba el error
  capaActiva = shapeType;  // Actualiza la capa activa
  ...
  switch (shapeType) {
    ...
    case 'estados':
      cargarEstados();
      cargarTerminosBusqueda();  // Carga términos cuando son estados
      break;
    case 'municipios':
      cargarMunicipios(null);
      cargarTerminosBusqueda();  // Carga términos cuando son municipios
      break;
    case 'viento':
      cargarRasterViento();
      break;
    default:
      console.warn("Tipo de shape no reconocido.");
      break;
  }
}


// Función para limpiar las capas activas en el mapa
// Limpia las capas activas del mapa
function limpiarMarcadores() {
  // Limpiar marcadores, clústeres y círculos
  map.eachLayer(function (layer) {
    if (
  ...
  // También limpiar capas de estados, municipios y raster de viento
  ...
  if (capaRasterViento && map.hasLayer(capaRasterViento)) {
    map.removeLayer(capaRasterViento);
    capaRasterViento = null;
  }
}
...
function cargarRegiones() {
  regionesLayer = L.geoJson(json_regiones_poligon_0, {
    style: style_regiones_poligon_0_0,
    onEachFeature: pop_regiones_poligon_0,
  }).addTo(map);
}
...
// Cargar Estados
// Evento hover para Estados
function cargarEstados() {
  estadosLayer = L.geoJson(estados, {
    style: initialStyle,
    onEachFeature: function (feature, layer) {
      layer.bindTooltip(
        '<div class="custom-tooltip">' + feature.properties.NOMGEO + "</div>"
      ...
      layer.on("mouseout", function () {
        // Verificamos si estadosLayer existe y si el layer aún está en el mapa antes de resetear el estilo
        if (estadosLayer && map.hasLayer(estadosLayer)) {
          estadosLayer.resetStyle(layer);
        }
      });
...
function cargarMunicipios(cveEnt) {
  municipiosLayer = L.geoJson(municipios_mapa, {
    ...
    onEachFeature: function (feature, layer) {
      ...
      layer.on("mouseout", function () {
        // Verificamos si municipiosLayer existe y si el layer aún está en el mapa antes de resetear el estilo
        if (municipiosLayer && map.hasLayer(municipiosLayer)) {
          municipiosLayer.resetStyle(layer);
        }
      });
...
// Cargar Raster de Viento
function cargarRasterViento() {
  // Limpia otras capas antes de cargar el raster
  if (estadosLayer && map.hasLayer(estadosLayer)) {
    map.removeLayer(estadosLayer);
    estadosLayer = null;
  }
  ...
  capaRasterViento = L.imageOverlay(urlRaster, bounds, { opacity: 0.7 }).addTo(
    map
  );
}

loadShape("regiones"); // Carga las regiones por defecto al abrir el mapa


// Nueva función para cargar Electricidad, Líneas de Transmisión y Subestaciones
function cargarMarcadores() {
  // Limpiar marcadores y capas anteriores
  limpiarMarcadores();

  // 1. Cargar Electricidad
  CargaElectricidad();

  // 2. Cargar Líneas de Transmisión
  function getColorlt(voltaje) {
    if (voltaje >= 400) return 'blue';
    else if (voltaje === 230) return 'yellow';
    else if (voltaje >= 138 && voltaje <= 161) return 'green';
    else if (voltaje >= 60 && voltaje <= 115) return 'magenta';
    else if (voltaje >= 44 && voltaje < 132) return 'white';
    else if (voltaje < 13.2) return 'orange';
  }

  function stylelt(feature) {
    var voltaje = feature.properties.Voltaje_KV;
    return {
      color: getColorlt(voltaje),
      weight: 2,
      opacity: 1
    };
  }

  function highlightFeaturelt(e) {
    var layer = e.target;
    layer.setStyle({
      weight: 5,
      color: 'black',
  ...
  function resetHighlightlt(e) {
    geojson.resetStyle(e.target);
  }

  function onEachFeaturelt(feature, layer) {
    layer.on({
      mouseover: highlightFeaturelt,
      mouseout: resetHighlightlt
    });
    layer.bindPopup('<b>Nombre:</b> ' + feature.properties.Nombre + '<br><b>Voltaje:</b> ' + feature.properties.Voltaje_KV + ' kV');
  }

  var geojson = L.geoJson(lineastrans, {
    style: stylelt,
    onEachFeature: onEachFeaturelt
  }).addTo(map);

  // 3. Cargar Subestaciones
  var autolinker = new Autolinker({ truncate: { length: 30, location: 'smart' } });

  function pop_subestaciones_0(feature, layer) {
    let clickedLat, clickedLon;

    function updatePopupContent() {
      var popupContent = '<table>\
        <tr><th scope="row">Nombre: </th><td>' + (feature.properties['Name'] ? autolinker.link(feature.properties['Name']) : '') + '</td></tr>\
        <tr><th scope="row">Fase: </th><td>' + (feature.properties['Fase'] ? autolinker.link(feature.properties['Fase']) : '') + '</td></tr>\
        <tr><th scope="row">Voltaje (kV): </th><td>' + (feature.properties['Voltaje_KV'] ? autolinker.link(feature.properties['Voltaje_KV']) : '') + '</td></tr>\
        <tr><td><a class="street-view-link btn btn-cre-verde" href="http://maps.google.com/maps?q=&layer=c&cbll=' + clickedLat + ',' + clickedLon + '&cbp=11,0,0,0,0" target="_blank"><b> Ver vista de calle </b></a></td></tr>\
      </table>';
  ...
  function style_subestaciones_0_0() {
    return {
      pane: 'pane_subestaciones_0',
      radius: 4.0,
      opacity: 1,
  ...
  map.createPane('pane_subestaciones_0');
  map.getPane('pane_subestaciones_0').style.zIndex = 400;

  var layer_subestaciones_0 = L.geoJson(json_subestaciones_0, {
    interactive: true,
    onEachFeature: pop_subestaciones_0,
    pointToLayer: function (feature, latlng) {
      return L.circleMarker(latlng, style_subestaciones_0_0(feature));
    }
  });

  bounds_group.addLayer(layer_subestaciones_0);
  map.addLayer(layer_subestaciones_0);
}
